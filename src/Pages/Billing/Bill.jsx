...
const Bill = () => {
  ...
  const handlePrint = () => {
    ...
    const monthText = months.join("/")
    const currentYear = new Date().getFullYear()
    const billNumber = `বিল নং-${monthText}-${currentYear}-1426`
    const totalBodyFare = selectedData.reduce((sum, dt) => sum + (Number.parseFloat(dt.body_fare) || 0), 0)
    const totalRent = selectedData.reduce((sum, dt) => sum + (Number.parseFloat(dt.total_rent) || 0), 0)
    const totalBodyFareWords = numberToWords(totalBodyFare)
    const totalRentWords = numberToWords(totalRent)
    ...
    const newWindow = window.open("", "_blank")
    const html = `...` // HTML structure translated similarly (প্রয়োজন হলে পুরোটা অনুবাদ করা যাবে)
    newWindow.document.write(html)
    newWindow.document.close()
    newWindow.focus()
    newWindow.print()
          ...
          <table className="min-w-full text-sm text-left text-gray-900">
            <thead className="capitalize text-sm">
              <tr>
                ...
                <th className="border border-gray-700 px-2 py-1">ট্রাক নং</th>
                <th className="border border-gray-700 px-2 py-1">লোডিং পয়েন্ট</th>
                <th className="border border-gray-700 px-2 py-1">আনলোডিং পয়েন্ট</th>
                <th className="border border-gray-700 px-2 py-1">ভাড়া</th>
