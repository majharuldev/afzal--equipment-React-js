...
const AllUsers = () => {
  ...
  useEffect(() => {
    axios
      ...
      .catch((error) => {
        console.error("ড্রাইভার ডেটা লোড করতে সমস্যা:", error);
        setLoading(false);
      });
  }, []);

  if (loading) return <p className="text-center mt-16">ইউজার লোড হচ্ছে...</p>;
  ...
  const handleDelete = async (id) => {
    try {
      ...
      if (!response.ok) throw new Error("ইউজার ডিলিট করা যায়নি");

      setUsers((prev) => prev.filter((user) => user.id !== id));
      toast.success("ইউজার সফলভাবে ডিলিট হয়েছে", {
        position: "top-right",
        autoClose: 3000,
      });
  ...
  return (
    <main>
                  ...
                  <tr key={index} className="hover:bg-gray-50 transition-all">
                    ...
                    <td className="action_column">
                      <div className="flex gap-1 justify-center">
                        ...
                        <button onClick={() => { setSelectedUserId(user.id); setIsOpen(true); }} className="text-red-900 hover:text-white hover:bg-red-900 px-2 py-1 rounded shadow-md transition-all cursor-pointer">
                          <FaTrashAlt className="text-[12px]" />
                        </button>
                      </div>
                    </td>
      ...
      {isOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-[#000000ad] z-50">
          <div className="relative bg-white rounded-lg shadow-lg p-6 w-72 max-w-sm border border-gray-300">
            <button onClick={toggleModal} className="text-2xl absolute top-2 right-2 text-white bg-red-500 hover:bg-red-700 cursor-pointer rounded-sm">
              <IoMdClose />
            </button>
            <div className="flex justify-center mb-4 text-red-500 text-4xl">
              <FaTrashAlt />
            ...
            <p className="text-center text-gray-700 font-medium mb-6">
              আপনি কি এই ইউজারটি ডিলিট করতে চান?
            </p>
            <div className="flex justify-center space-x-4">
              <button onClick={toggleModal} className="bg-gray-100 text-gray-700 px-4 py-2 rounded-md hover:bg-primary hover:text-white cursor-pointer">
                না
              </button>
              <button onClick={() => handleDelete(selectedUserId)} className="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-red-700 cursor-pointer">
                হ্যাঁ
              </button>
            </div>
          </div>
